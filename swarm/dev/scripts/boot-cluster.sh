...
#
# The cluster contains a bootnode, a gbas node and multiple swarm nodes, with
...
# static gbas configuration
GBAS_IP="192.168.33.3"
GBAS_RPC_PORT="8515"
GBAS_RPC_URL="http://${GBAS_IP}:${GBAS_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gbas and
# swarm nodes
start_bootnode() {
...
# start_gbas_node starts a gbas node with --datadir pointing at <base-dir>/gbas
# and a single, unlocked account with password "gbas"
start_gbas_node() {
  local dir="${base_dir}/gbas"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gbas account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GBAS_IP}"
    --rpcport   "${GBAS_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gbas" "${GBAS_IP}" "$(which gbas)" ${args[@]}
}
...
create_account() {
  ...
  gbas --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
