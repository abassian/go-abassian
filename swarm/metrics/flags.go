...
import (
	...
	gbasmetrics "github.com/abassian/go-abassian/metrics"
...
func Setup(ctx *cli.Context) {
	if gbasmetrics.Enabled {
		...
		go gbasmetrics.CollectProcessMetrics(2 * time.Second)
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gbasmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", tagsMap)
		}

		if enableAccountingExport {
			log.Info("Exporting swarm accounting metrics to InfluxDB")
			go influxdb.InfluxDBWithTags(gbasmetrics.AccountingRegistry, 10*time.Second, endpoint, database, username, password, "accounting.", tagsMap)
		}
	}
}
