...
[https://swarm.abassian.org](https://swarm.abassian.org)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/abassian/go-abassian

    go install github.com/abassian/go-abassian/cmd/swarm
...
To run Swarm you need an Abassian account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the Abassian address representing that account. For example:
...
### Abassian Name Service resolution

The Abassian Name Service is the Abassian equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.eth`), `swarm` has to connect to a `gbas` instance, which is synced with the Abassian mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.abassian/gbas.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/abassian/go-abassian`.

Most likely you will be working from your fork of `go-abassian`, let's say from `github.com/nirname/go-abassian`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-abassian.git $GOPATH/src/github.com/abassian/go-abassian
```
...
```
stateth --rm --grafana-dashboards-folder $GOPATH/src/github.com/abassian/go-abassian/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The Abassian Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
 ...
 * [Code review guidelines](https://github.com/abassian/go-abassian/wiki/Code-Review-Guidelines).
