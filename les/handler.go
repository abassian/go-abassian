...
import (
	...
	"github.com/abassian/go-abassian/common/mclock"
...
// NewProtocolManager returns a new ethereum sub protocol manager. The Abassian sub protocol manages peers capable
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Abassian protocol")
	...
	log.Info("Light Abassian protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Abassian peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Abassian handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Abassian peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Abassian message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Abassian message arrived", "code", msg.Code, "bytes", msg.Size)
