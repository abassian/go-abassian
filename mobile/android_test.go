...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gbas.LightScryptN, Gbas.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.abassian", new NodeConfig());
			node.start();
			...
			AbassianClient ec = node.getAbassianClient();
	...
	// signatures too. Regression test for go-abassian issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogbaser
	...
	workspace, err := ioutil.TempDir("", "gbas-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethereum/gbastest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gbas and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.abassian", "github.com/abassian/go-abassian/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gbas.aar"), "gbas.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethereum", "gbastest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
