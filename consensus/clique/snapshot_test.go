...
// mapped from textual names used in the tests below to actual Abassian private
...
// address retrieves the Abassian address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Abassian address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/abassian/go-abassian/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Gbas the header and prepare it for signing
			header := block.Header()
			if j > 0 {
