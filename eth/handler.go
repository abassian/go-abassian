...
import (
	...
	"github.com/abassian/go-abassian/eth/fetcher"
...
// NewProtocolManager returns a new Abassian sub protocol manager. The Abassian sub protocol manages peers capable
// with the Abassian network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Abassian peer", "peer", id)

	// Unregister the peer from the downloader and Abassian peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Abassian protocol")
	...
	log.Info("Abassian protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Abassian peer connected", "name", p.Name())

	// Execute the Abassian handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Abassian handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Abassian peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Abassian message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Abassian sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Abassian network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
