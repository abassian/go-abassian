...
import (
	...
	"github.com/abassian/go-abassian/accounts/usbwallet/internal/trezor"
...
// or whether the Abassian app was not started on it.
...
// Abassian address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AbassianAddress)
	if _, err := w.trezorExchange(&trezor.AbassianGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.AbassianTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AbassianTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
