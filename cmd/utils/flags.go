...
// Package utils contains internal helper functions for go-abassian commands.
package utils
...
import (
	...
	"github.com/abassian/go-abassian/common/fdlimit"
	...
	"github.com/abassian/go-abassian/dashboard"
	...
	"github.com/abassian/go-abassian/metrics"
	"github.com/abassian/go-abassian/metrics/influxdb"
	"github.com/abassian/go-abassian/node"
	"github.com/abassian/go-abassian/p2p"
	"github.com/abassian/go-abassian/p2p/discv5"
	"github.com/abassian/go-abassian/p2p/enode"
	"github.com/abassian/go-abassian/p2p/nat"
	"github.com/abassian/go-abassian/p2p/netutil"
...
// for Gbas and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gbas account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Abassian client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Abassian service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Abassian Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Abassian
		ctx.Service(&ethServ)
		...
		Fatalf("Failed to register the Abassian Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gbas.", tagsMap)
		}
	}
}
...
// new format.
//
// e.g. gbas account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gbas --keystore /tmp/mykeystore --lightkdf account new
//
